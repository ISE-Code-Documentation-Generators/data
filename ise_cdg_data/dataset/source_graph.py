    ...
    def __len__(self):
        if not hasattr(self, "df"):
            raise NotImplementedError(
                "You must either fill the `df` field or override the `__len__` method"
            )
        return len(self.df)
...
class SourceGraphDatasetWithPreprocess(SourceGraphDataset):
    ...
    def node2identifier(self, node):
        possible_identifiers = ["asname", "name", "attr", "id", "arg"]
    ...
    def node2feature(self, node):
        ...
        return self.src_vocab.vocab.get_stoi().get(
            id_, self.src_vocab.vocab.get_stoi()["<unk>"]
        )
    ...
    def get_nodes_edges_rep(self, index) -> typing.Tuple[torch.Tensor, torch.Tensor]:
        ...
        edges_rep = torch.Tensor(
            [(nodes.index(e[0]), nodes.index(e[1])) for e in graph.edges]
        )
...
class SourceGraphDatasetWithoutPreprocess(SourceGraphDataset):
    ...
    def get_nodes_edges_rep(self, index) -> typing.Tuple[torch.Tensor, torch.Tensor]:
        return torch.Tensor(self.nodes.iloc[index]), torch.Tensor(
            self.edges.iloc[index]
        )
